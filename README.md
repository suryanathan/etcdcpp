...
Forked from: https://github.com/coreos/etcd/blob/release-2.2/Documentation/api.md

Every api call supported by etcdcpp is specified next to the curl calls
...
Create an instance of etcd::Client. RapidReply is a json wrapper. Take a look at rapid_reply.hpp.
It can easily be wrapped to any custom json library.
...
```
In our example rapid reply wrapper, we parse for an error and throw an exception of type etcd::ReplyException
```sh
...
```cpp
example::RapidReply reply = etcd_client.Set("/foo", "bar");
```
...
```cpp
etcd_watchdog.RunOnce("/foo", WatchCallback, 7);
```
...
```
```cpp
example::RapidReply reply = etcd_client.SetOrdered("/queue", "Job2");
```
...
```
```cpp
example::RapidReply reply = etcd_client.Set("/foo", "one");
```
...
The example::RapidReply wrapper also throws an exception
...
```cpp
example::RapidReply reply = etcd_client.CompareAndSwapIf("/foo", "two", std::string("one"));
```
...
```
```cpp
example::RapidReply reply = etcd_client.CompareAndDeleteIf("/foo", std::string("one"));
```
...
```
```cpp
example::RapidReply reply = etcd_client.AddDirectory("/foo_dir/foo");
```
...
```
```cpp
example::Reply reply = etcd_client.Get("/");
```
